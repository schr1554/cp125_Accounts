...
public final class Invoice {
	...
	private String invoicingBusinessName() {
		StringBuilder businessInvoiceBuilder = new StringBuilder();
		return businessInvoiceBuilder.append(prop.get("business.name")).toString();
	}

	private Address invoicingBusinessAddress() {

		Address address = new Address(prop.get("business.street").toString(), prop.get("business.city").toString(),
				StateCode.valueOf(prop.getProperty("business.state")), prop.get("business.zip").toString());
	...
	public void extractLineItems(TimeCard timeCard) {
		...
		for (ConsultantTime consultingHour : timeCard.getConsultingHours()) {
			...
			if (consultingHour.getDate().getMonth() == this.invoiceMonth) {
				...
				if (consultingHour.getAccount().equals(this.client)) {
					if (consultingHour.isBillable()) {
						addLineItem(new InvoiceLineItem(consultingHour.getDate(), timeCard.getConsultant(),
	...
	public String toString() {
		...
		invoiceString.append(invoicingBusinessName());
	...
	public String toReportString() {
		...
		InvoiceFooter footer = new InvoiceFooter(invoicingBusinessName());
		InvoiceHeader header = new InvoiceHeader(invoicingBusinessName(), invoicingBusinessAddress(), this.client,
				getStartDate(), getInvoiceDate());
